    ...
    for item in property_constraints:
        if item in constraint_shacl:
            if item == "Q21503250":
                ...
                constraint_shape = constraint_shape.replace("$WD_PROPERTY$", wdProperty)
                for json_item in property_json["results"]["bindings"]:
                    if (
                        json_item.get("constraint_type").get("value")[
                            json_item.get("constraint_type").get("value").rfind("/Q")
                            + 1 :
                        ]
                        == item
                        ...
                        constraint_shape = constraint_shape.replace("$PROPERTY_SHACL$", "")
                with open(f"{results_path}/shacl_shape_{wdProperty}_{item}.ttl", "w") as outfile:
